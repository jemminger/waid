...
export class SqliteProvider implements TaskProvider {
  async getAll(): Promise<Task[]> {
    const db = await getDb();
    return db.select<Task[]>(
      'SELECT * FROM tasks WHERE deleted_at IS NULL ORDER BY position ASC'
    );
  }

  async create({ name, details }: { name?: string | null; details?: string }): Promise<Task> {
    const db = await getDb();
  ...
  async update(id: number, { name, details }: { name?: string | null; details?: string }): Promise<Task> {
    const db = await getDb();
    await db.execute(
